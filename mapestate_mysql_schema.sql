-- MapEstate MySQL Database Schema
...
--   mysql -u root -p mapestate < mapestate_mysql_schema.sql
...
-- Procedure to get property with details
CREATE PROCEDURE GetPropertyDetails(IN property_id VARCHAR(36))
BEGIN
  SELECT 
    p.*,
    u.username as agent_username,
...
CREATE PROCEDURE SearchProperties(
  ...
  IN search_type VARCHAR(255),
  IN search_listing_type VARCHAR(255),
...
BEGIN
  ...
  WHERE 
    (search_city IS NULL OR city LIKE CONCAT('%', search_city, '%'))
    AND (search_type IS NULL OR type = search_type)
    AND (search_listing_type IS NULL OR listing_type = search_listing_type)
